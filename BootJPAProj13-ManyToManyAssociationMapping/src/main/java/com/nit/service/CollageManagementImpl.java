...
public void deleteDataUsingFaculty(Integer id) {
	...
	if(byId.isPresent())
	{
		...
		System.out.println("Faculty with assigned student deleted of id "+id);
	}
	else
	{
...
public void deleteDataUsingStudent(Integer id) {
	Optional<Student> studId = studentRepo.findById(id);
	if(studId.isPresent())
	{
		Student student = studId.get();
		Set<Faculty> facultys = student.getFacultyInfo();
		student.setFacultyInfo(null);
		facultys.forEach(fac->{fac.getStudentInfo().remove(student);
		facultyRepo.save(fac);});
		//studentRepo.save(student);
		System.out.println("Student with Id "+id+" and it's associted faculty deleted");
	}
	else
	{
...
public void showStudentPerfaculty(Integer id) {
	...
	List<Object[]> list = facultyRepo.findStudentPerFacultyUsingJoin(id);
	
	list.forEach(fac->System.out.println(Arrays.toString(fac)));
}//method

@Override
public void showfacultyPerStudent(Integer id) {
	studentRepo.findFacultyPerStudentUsingJoin(id)
	.forEach(stud->System.out.println(Arrays.toString(stud)));
}
